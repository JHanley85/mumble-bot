...
pub fn tcp_recv_loop<'a>(
    ...
    loop_fn((tcp_socket_rx, tcp_tx, session, crypt),
            |(tcp_socket_rx, tcp_tx, session, crypt)| {
        ...
        .and_then(|(tcp_socket_rx, buf, mum_type)| {
            ...
            match mum_type {
                0 => { // Version
                    ...
                    println!("version: {:?}", msg);
                    ok((tcp_socket_rx, tcp_tx, session, crypt)).boxed()
                },
                1 => { // UDPTunnel
                    let mut rdr = Cursor::new(&buf);
        ...
        .and_then(move |(tcp_socket_rx, tcp_tx, session, crypt)| {
            Ok(Loop::Continue((tcp_socket_rx, tcp_tx, session, crypt)))
        })
    })
}
